...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const fundearchInput = document.getElementById("fundsearch")
  
  // actual fundsearch logic
  const filterItems = (searchTerm) => {
    document.querySelectorAll(".funding, .unloaded").forEach((element) => element.classList.remove("unloaded"));
    
    // highlight-search-term
    if (CSS.highlights) {
      const nonMatchingElements = highlightSearchTerm({ search: searchTerm, selector: ".funding > li" });
      ...
      document.querySelectorAll(".funding > li").forEach((element, index) => {
    ...
    document.querySelectorAll("h2.funding-year").forEach(function (element) {
  ...
  const updateInputField = () => {
    ...
    document.getElementById("fundsearch").value = hashValue;
    filterItems(hashValue);
  };

  // Sensitive search. Only start searching if there's been no input for 300 ms
  let timeoutId;
  fundearchInput.addEventListener("input", function () {
